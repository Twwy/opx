...
class database {
	private $dbHost = SAE_MYSQL_HOST_M;
	private $dbName = SAE_MYSQL_DB;
	private $dbUser = SAE_MYSQL_USER;
	private $dbPass = SAE_MYSQL_PASS;
	private $dbPort = SAE_MYSQL_PORT;
	private $dbObj = false;

	function __construct(){
		//$this->dbObj = new PDO("mysql:host={$this->dbHost};dbname={$this->dbName};charset=UTF-8", $this->dbUser, $this->dbPass);
		$this->dbObj = new PDO("mysql:host={$this->dbHost};dbname={$this->dbName};charset=UTF-8;port={$this->dbPort}", $this->dbUser, $this->dbPass, Array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES'UTF8';"));
